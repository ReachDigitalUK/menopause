...
class Image
{
        ...
        if ($image_url = self::URL($args['name'])) {
            ...
            if (!isset($attributes['width']) || !isset($attributes['height'])) {
                ...
                if (pathinfo($name, PATHINFO_EXTENSION) === 'svg') {
                    $svgInfo = \Reach\SVG::info(\Reach\SVG::path($args['name']));
            ...
            $image = '<img ' . \Reach\Helpers::buildAttributes($attributes) . '>';
        }
    ...
    {
        return \Reach\Asset::URL('images/' . $name);
    }
    ...
    {
        return \Reach\Asset::path('images/' . $name);
    }
}
