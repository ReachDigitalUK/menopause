...
export default function watch() {
    ...
    gulp.watch(
        ...
        gulp.series(stylesTheme, stylesComponents, assetManifest, stylesLint)
    );

    // Scripts
    gulp.watch(
        `${config.paths.base.src}**/*.js`,
        gulp.series(scriptsTheme, scriptsComponents, assetManifest, scriptsLint)
    );

    // Static
    gulp.watch(`${config.paths.staticAssets.src}**/*`, { cwd: config.paths.base.src }, staticAssets);

    // Images
    gulp.watch(`${config.paths.base.src + config.paths.images.src}**/*`, gulp.series(images, assetManifest));

    // SVGs
    gulp.watch(`${config.paths.base.src + config.paths.images.src}**/*.svg`, gulp.series(svg, assetManifest));
