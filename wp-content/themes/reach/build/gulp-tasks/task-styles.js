...
function taskStylesFix(src, dest) {
    ...
    return gulp.src(src).pipe(plumber()).pipe(stylelint(config.plugins.stylelint)).pipe(gulp.dest(dest));
}
 ...
 *
 * When compiling all theme styles, only the _src/ directory root files (_src/main.scss, _src/editor.scss, etc)
